...
<title>编辑费用</title>
	...
	<script>
		$(function(){
			...
			$.each($("#wrap td input"),function(i,n){
				//alert($(this).attr("type"));
				$(this).attr("onkeypress","return event.keyCode>=48&&event.keyCode<=57||event.keyCode==46");
				$(this).attr("onpaste","return !clipboardData.getData('text').match(/\D/)");
...
<body>
	<div id="wrap">
		<form method="post" name="costForm" id="costForm" action="/security/cost.htm?method=saveYs">
			<%--<div id="wrap_head">
				<table width="100%" border=0>
					<tr>
						<td align="left" width="25%">
							<a href="/security/cost.htm?method=editYs&tag=0&currentYear=${_Year}"><<前一年</a>
						</td>
						<td align="center">
							<span style="font-size:18px;font-weight:bold;">${_Year}年度安全费用预算</span>
							<input type="hidden" name="dataYear" value="${_Year}" />
						</td>
						<td align="right" width="25%">
							<c:if test="${_Year != _RealYear }">
								<a href="/security/cost.htm?method=editYs&tag=1&currentYear=${_Year}">后一年>></a>
							</c:if>
							<c:if test="${_Year == _RealYear }">
								&nbsp;
							</c:if>
			...
			--%><table width="100%" border="0" cellpadding="5" cellspacing="1" bgcolor="silver">
	            ...
	            <tr>
	              ...
	              <td width="7%" align="center"><input type="text" name="1_1" id="1_1" value="${v1_1}"  onkeypress = "return event.keyCode>=48&&event.keyCode<=57||event.keyCode==46" onpaste = "return !clipboardData.getData('text').match(/\D/)" ondragenter = "return false" style= "ime-mode:Disabled" /></td>
	         ...
	         <div id="wrap_foot" style="text-align:left;padding:20px;">
				...
				&nbsp;&nbsp;&nbsp;&nbsp;
				<a href="javascript:void(0);" class="widget" onclick="javascript:toExcel('dataTab','年度预算');"><span>导出Excel</span></a>
			</div>
	        </div>
		</form>
