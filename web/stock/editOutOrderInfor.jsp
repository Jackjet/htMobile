...
<title>编辑出库单</title>
	...
	<script>
	    ...
	    function displayDetail(id){
	    	...
	    	//alert($("#"+id).find("option:selected").attr("value"));
	    	//alert(itemUnit + "--"+itemPrice+"-"+priceUnit+"--"+itemCount);
	    	...
	    	$.getJSON("/stock/supplierInfor.htm?method=getOutSuppliers&itemId="+$("#"+id).val(),function(data) {
		           ...
		           $.each(data._SupplierItems, function(i, n) {
		               options += "<option value='"+n.supplierId+"'  class='"+n.restCount+"'>"+n.supplierName+"</option>";
		               //+"|"+n.inOrderItemId+"   
		           });   
		           if(options == ""){
		           	$('#supplierId').val("");
		           //	alert($("#supplierId + .custom-combobox > .custom-combobox-input").val())
		           	$("#supplierId + .custom-combobox > .custom-combobox-input").val("");
		           	$("#canUseCountDisplay").text("0");
		           }
	    ...
	    function setVehicles(id){
	    	...
	    	if(deviceVal == "F"){
				...
				<c:forEach var="device" items="${_Devices}" varStatus="index">
					...
					deviceArray[${index.index}] = deviceCode;
				</c:forEach>
	     ...
	     function countAmount(){
	     	...
	     	if(parseFloat(count) > parseFloat(canUseCount)){
				...
				//    itemAmount = str;
		     	//}
		     	//if(count > 1){
		     		itemAmount = (count * itemPrice).toFixed(2);
		     	//}
					  			...
					  			<form:form commandName="outOrderInforVo" id="outOrderInforForm" action="/stock/outOrderInfor.htm?method=save" method="post">
									...
									<table width="100%" border="0" cellpadding="0" cellspacing="0" class="doc" style="border:0px solid red;">
										...
										<tr>
											...
											<td>
												<form:select path="typeId" class="select" url="123" ignore="ignore" datatype="*" nullmsg="请选择分类！" sucmsg="选择正确！">
						                      		<form:option value="">--选择分类--</form:option>
						                      		<c:forEach items="${_Types}" var="type">
														<form:option value="${type.typeId}">【${type.typeCode}】${type.typeName}</form:option>
													</c:forEach>
						                      	</form:select>
						                      	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
												<!--<font color="red">*</font>
											...
											<td>
												<form:select path="deviceId" class="select" datatype="*" nullmsg="请选择设备！" sucmsg="选择正确！" onchange="setVehicles('deviceId');">
						                      		...
						                      		<c:forEach items="${_Devices}" var="device">
														<form:option value="${device.deviceId}">${device.deviceCode}</form:option>
													</c:forEach>
						                      	</form:select>
						                      	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
												<font color="red">*</font>
										...
										<!--<tr>
											<td align="right"><span class="blues">备件小类：</span>&nbsp;&nbsp;</td>
											<td colspan=3>
										...
										--><tr>
											...
											<td colspan=3>
												<input class="textfield" id="searchWord" size="10" onkeydown="rtnItems('searchWord');" onblur="searchItems('searchWord');"/>
										...
										<tr>
											...
											<td>
												...
												<form:input path="outItemPrice" size="20" class="textfield" onblur="countAmount();" readonly="true" />
