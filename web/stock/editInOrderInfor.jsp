	...
	<script>
	    ...
	    function addItem(){
	    	//if($("#itemForm").Validform()){
		    	...
		    	//计算目前订单总金额
		    	var pureAmount = calPureAmount();
		    	$("#pureAmount").val(pureAmount);
	  ...
	  //计算目前订单总金额
	    function calPureAmount(){
	    	var pureAmount = 0;
	    	$("#itemsTab-real .itemAmounts").each(function(i,n){
	    		//alert($(this).val());
	    		pureAmount += parseFloat($(this).val());
	    	});
	    	pureAmount = pureAmount.toFixed(4);
	    ...
	    function deleteTr(num){
	     ...
	     function countAmount(id){
	     	...
	     	//alert(itemAmount);
	     	$("#itemAmount").val(itemAmount.toFixed(5));
	     }
					  			...
					  			<form:form commandName="inOrderInforVo" id="inOrderInforForm" action="/stock/inOrderInfor.htm?method=save" method="post">
									...
									<table width="100%" border="0" cellpadding="0" cellspacing="0" class="doc" style="border:0px solid red;">
										...
										<tr>
											...
											<td>
												<form:input path="amount" size="20" class="textfield" datatype="*,/^\s*$/|/^[0-9]{1,}\.{0,1}[0-9]{0,5}0*$/" sucmsg="填写正确！" errormsg="请输入正确格式" nullmsg="请输入订单总金额（含税）" />
											</td>
										</tr>
										<tr>
											<td nowrap="nowrap" align="right"><span class="blues">订单总金额（去税）：</span>&nbsp;&nbsp;</td>
											<td>
												<form:input path="pureAmount" size="20" class="textfield" datatype="*,/^\s*$/|/^[0-9]{1,}\.{0,1}[0-9]{0,5}0*$/" ignore="ignore" sucmsg="填写正确！" errormsg="请输入正确格式" nullmsg="请输入订单总金额（去税）" />
											</td>
										</tr>
										<tr>
										...
										<tr>
											...
											<td style="vertical-align:bottom;border:0px solid red;">
												<table id="itemsTab-real" width="90%" border="0" cellpadding="0" cellspacing="1" bgcolor="#b4c5cd" class="doc">
													...
													<c:if test="${!_IsNew}">
														<script>
															tempnum=${fn:length(_InOrder.items)};
															tempsum=${fn:length(_InOrder.items)};
														</script>
														<c:forEach items="${_InOrder.items}" var="inOrderItem" varStatus="index">
															<tr class="contentTr itemsTabTr${index.index+1}">
																<td><input type="hidden" name="itemSum" value="${index.index+1}" />${index.index+1}</td>
																<td>
																	<span id="itemName${index.index+1}">【${inOrderItem.itemInfor.itemCode}】${inOrderItem.itemInfor.itemName}</span>
																	<input type="hidden" name="itemId${index.index+1}" id="itemId${index.index+1}" value="${inOrderItem.itemInfor.itemId}" />
																</td>
																<td>
																	<span id="itemUnit${index.index+1}">${inOrderItem.itemInfor.itemUnit}</span>
																</td>
																<td>
																	<input type="input" class="textfield" value="${inOrderItem.acceptCount}" size="5" name="acceptCount${index.index+1}" readonly="readonly" />
																</td>
																<td>
																	<input type="input" class="textfield" value="${inOrderItem.inItemPrice}" size="10" name="itemPrice${index.index+1}" readonly="true" />&nbsp;(<span id="itemPriceUnit${index.index+1}">${inOrderItem.itemInfor.priceUnit}</span>)
																</td>
																<td>
																	<input type="input" class="textfield" value="${inOrderItem.becomeDate}" size="12" name="becomeDate${index.index+1}" readonly="true" />
																</td>
																<td>
																	<input type="input" class="textfield itemAmounts" value="${inOrderItem.amount}" size="10" name="itemAmount${index.index+1}" readonly="true" />
																</td>
																<td>
																	<img src="/images/button/delete.png" align="absMiddle" title="删除此行" style="cursor:pointer;border:0;" onclick="deleteTr('${index.index+1}');" />
																</td>
															</tr>
														</c:forEach>
									...
									<div id="buttons">
										<table width="300" border="0" align="center" cellpadding="0"
											...
											<tr>
												<td>
													<a href="javascript:void(0);" class="btn_blue" id="btn_sub"><span>保存不入库</span></a>
												</td>
												<td>
									...
									<!-- <div style="width:60%;height:auto;border:1px solid silver;"> -->
										<table width="100%" border="0" cellpadding="0" cellspacing="0" class="doc" style="border:0px solid red;">
												...
												<td style="vertical-align:bottom;border:0px solid red;">
													<table id="itemsTab" width="90%" border="0" cellpadding="0" cellspacing="1" bgcolor="#b4c5cd" class="doc">
														...
														<c:if test="${!_IsNew}">
															...
															<c:forEach items="${_InOrder.items}" var="inOrderItem" varStatus="index">
																<tr class="contentTr itemsTabTr${index.index+1}">
																	<td><input type="hidden" name="${index.index+1}" value="${index.index+1}" />${index.index+1}</td>
																	<td>
																		<span id="itemName${index.index+1}">【${inOrderItem.itemInfor.itemCode}】${inOrderItem.itemInfor.itemName}</span>
																		<input type="hidden" name="itemId${index.index+1}" id="itemId${index.index+1}" value="${inOrderItem.itemInfor.itemId}" />
																	</td>
											...
											<tr>
												...
												<td align="left">
													<br/>
													<div style="width:80%;height:auto;border:1px solid silver;">
														<table width="100%" border="1" cellpadding="0" cellspacing="5" class="doc" style="border:0px solid red;">
															...
															--><tr>
																<td colspan=2>&nbsp;&nbsp;备&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;件：
																	...
																	<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
																	<select name="itemId" id="itemId" class="select" datatype="*" nullmsg="请选择备件！" sucmsg=" " onchange="displayDetail('itemId');" >
															...
															<tr>
																<td width="35%">
																	&nbsp;&nbsp;单&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;位：<span id="itemUnit"></span>
																</td>
																<td>
																	单&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;价：
																	<input type="input" class="textfield" size="10" onblur="countAmount('acceptCount');" id="inItemPrice" name="inItemPrice" datatype="*,/^\s*$/|/^[0-9]{1,}\.{0,1}[0-9]{0,5}0*$/" sucmsg="填写正确！" errormsg="请输入正确格式" nullmsg="请输入单价" />
																	<!--<span id="itemPrice"></span>-->
																	 (<span id="itemPriceUnit"></span>)
																</td>
															</tr>
															<!-- <tr>
																<td>&nbsp;&nbsp;申请数量：<input type="input" class="textfield" onblur="countAmount('applyCount');setSameCount('applyCount','buyCount','acceptCount');" size="7" id="applyCount" name="applyCount" value="1" datatype="*,/^\s*$/|/^[0-9]{1,}\.{0,1}[0-9]{0,5}0*$/" sucmsg="填写正确！" errormsg="请输入正确格式" nullmsg="请输入申请数量" /></td>
																<td>采购数量：<input type="input" class="textfield" size="7" onblur="countAmount('buyCount');setSameCount('buyCount','applyCount','acceptCount');" id="buyCount" name="buyCount" value="1" datatype="*,/^\s*$/|/^[0-9]{1,}\.{0,1}[0-9]{0,5}0*$/" sucmsg="填写正确！" errormsg="请输入正确格式" nullmsg="请输入采购数量" /></td>
															</tr> -->
															<tr>
																<td>&nbsp;&nbsp;入库数量：<input type="input" class="textfield" size="7" onblur="countAmount('acceptCount');setSameCount('acceptCount','applyCount','buyCount');" id="acceptCount" name="acceptCount" value="1" datatype="*,/^\s*$/|/^[0-9]{1,}\.{0,1}[0-9]{0,5}0*$/" sucmsg="填写正确！" errormsg="请输入正确格式" nullmsg="请输入接收数量" /></td>
																<td>到期日期：<input type="input" class="Wdate textfield" size="15" id="becomeDate" name="becomeDate" readonly="true" datatype="*" sucmsg="填写正确！" nullmsg="请选择到期日期！" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" value="${_Today}"/></td>
															</tr>
															<tr>
																<td colspan=2>&nbsp;&nbsp;总&nbsp;金&nbsp;额&nbsp;：<input type="input" class="textfield" size="10" id="itemAmount" name="itemAmount" datatype="*,/^\s*$/|/^[0-9]{1,}\.{0,1}[0-9]{0,5}0*$/" sucmsg="填写正确！" errormsg="请输入正确格式" nullmsg="请输入总金额" /></td>
															</tr><!--  onfocus="countAmount();" -->
															<tr>
																<td colspan=2>
