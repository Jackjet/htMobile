//  document.write( "<script language= 'javascript' src='commonFunction.js'></script>"); 
		...
		if (attachment!=null && attachment!=""){
			...
			for(var k =0; k<filePaths.length;k++){
				...
				if (tempFile.length > 0) {
					var first = tempFile.indexOf("|");
					var second = tempFile.lastIndexOf("|");
					var uPath = tempFile.substring(0,first);
					var fileName = tempFile.substring(first+1,second);
					var size = tempFile.substring(second+1);
					var t = uPath + "/" + fileName;
				
					returnAttachment += "<a style=\"text-decoration:none\" href=\"" + contextPath + "/common/download.jsp?filepath=";				
					//alert(URLEncode(tempFile));
					returnAttachment += URLEncode(t);
					...
					returnAttachment += "<img title=\"点击下载：" + fileName + "\" width=\"16\" height=\"16\"  border=\"0\" src=\"" + contextPath + "/images/";
					//根据后缀显示不同的图片׺
					var suffix="";
					...
					if(suffix!=""){
						...
						if(suffix == "DOC" || suffix == "DOCX"){
							returnAttachment += "word.gif\"";
						}else if(suffix == "PDF"){
							returnAttachment += "pdf.gif\"";
						}else if(suffix == "XLS" || suffix == "XLSX"){
							returnAttachment += "excel.gif\"";
						}else if(suffix == "TXT"){
							returnAttachment += "txt.gif\"";
						}else{
							returnAttachment += "unknow.gif\"";
						}
					}
					returnAttachment += ">";
					
					returnAttachment += fileName;
					returnAttachment += "</a>";
				}
			}
		}
