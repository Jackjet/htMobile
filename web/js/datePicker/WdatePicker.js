/*
 * My97 DatePicker 4.1 SITE: http://dp.my97.net BLOG: http://my97.cnblogs.com
...
(function() {
	...
	if ($.$crossFrame) {
		try {
			while (T.parent[N] != T[N]
					&& T.parent[N][A]("frameset").length == 0)
				T = T.parent
		} catch (P) {
		}
	}
	...
	if (!T.$dp || L) {
		$dp = Q({
					...
					defMaxDate : $.maxDate,
					$ : function($) {
						return (typeof $ == "string") ? this.win[N]
								.getElementById($) : $
					},
					$D : function($, _) {
	...
	function M($) {
		$ = $ || T;
		var _ = $[N];
		_ = _[H] && _[H].clientHeight
				&& _[H].clientHeight <= _.body.clientHeight ? _[H] : _.body;
		return {
			"width" : _.clientWidth,
	...
	function S(C, $) {
		...
		if ($) {
			...
			if (!C.el
					|| C.el
					&& C.el.disabled
					|| (C.el == $dp.el && $dp.dd.style.display != "none" && $dp.dd.style.left != "-1970px"))
				return;
			I(C)
		}
		function _() {
	...
	function I(G, A) {
		...
		for (F in G)
			if ($dp[F] === undefined)
				...
				$dp[F] = G[F];
		$dp.elProp = $dp.el && $dp.el.nodeName == "INPUT"
				? "value"
				: "innerHTML";
