...
@Service("documentCategoryManager")
public class DocumentCategoryManagerImpl extends BasicManagerImpl<DocumentCategory> implements DocumentCategoryManager{
	...
	@Autowired
	public void setSystemDocumentCategoryDAO(DocumentCategoryDAO documentCategoryDAO) {
	...
	//把某个分类的所有子类id按照1,2,3的格式组合
		...
		public ArrayList getCategoryAsTree(Integer categoryId) {
			...
			List allCategorys = this.documentCategoryDAO.getAllValid();
		...
		private void addSubCategoryToArray(ArrayList array, DocumentCategory category, User user,List allCategorys) {
...
//				}
		...
		// 获取用户有权限看到的分类列表，树状显示
		public ArrayList getCategoryAsTree(Integer categoryId, User user) {
			ArrayList arrayCategory = new ArrayList();
			List allCategorys = this.documentCategoryDAO.getAll();
		...
		private int getSumOfSubCategory(DocumentCategory category,List allCategorys,User user){
			int number = 0;
...
//					}
		 ...
		 * 
		 */
		public String getSubCategoryIds(User user, int categoryId) {
			String ids = "";
			DocumentCategory category = (DocumentCategory) this.documentCategoryDAO.get(categoryId);
		...
		private String addSubCategoryId(User user, DocumentCategory category) {
			List childs = new ArrayList(category.getChilds());
			Iterator it = childs.iterator();
...
//					User user = (User) this.systemUserManager.get(userId);
//
