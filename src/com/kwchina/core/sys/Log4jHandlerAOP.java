...
public class Log4jHandlerAOP {
	...
	//拦截登录信息
    @After("execution(* com.kwchina.work.sys.LoginConfirm.login(..)) && args(request,..)")
    public void loginLog(JoinPoint joinPoint, HttpServletRequest request) {

        User systemUser = SysCommonMethod.getSystemUser(request);
        if (systemUser != null) {
        	//登录时间
...
//        System.out.println("----执行service拦截----");
	...
	//拦截退出信息
    @Before("execution(* com.kwchina.work.sys.Logout.logout(..)) && args(request,..)")
