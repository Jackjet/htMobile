...
public class BasicManagerImpl<T> implements BasicManager { 
	...
	//根据sql查找指定数量的结果
	public List<T> getResultByQueryString(final String queryString, final int firstResult, final int maxResults) {
		return this.basicDAO.getResultByQueryString(queryString, true, firstResult, maxResults);
	}
	...
	public int getMaxId(String idName){
		return this.basicDAO.getMaxId(idName);
	}
	
	public void doSqlDelete(String sql) {
		this.basicDAO.doSqlDelete(sql);
	}
}
